...
public void waitForElementToBeVisible(By path, int timeoutInSeconds) {
    try {
        new WebDriverWait(driver, Duration.ofSeconds(timeoutInSeconds)).until(ExpectedConditions.visibilityOfElementLocated(path));
    } catch (Exception e) {
          }
    }
...
/*  A. Method name : scrollByPath
    B. Author name : Logisetty Nischith
    C. Description : Method to scroll down By Path
    D. Return type : void
    E. Parameter : Null   */
public void scrollByPath(By path){
    WebElement element= driver.findElement(path);
    JavascriptExecutor js = (JavascriptExecutor) driver;
